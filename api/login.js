...
export default async function handler(req,res) {
 ...
 try {
  ...
  if (!usuario || usuario.length>254 || typeof senha !== 'string' || !senha || senha.length>200) throw falha(400,'Informe e-mail e senha.');
  await limitarIP(req,'login',50); await limitar(`login:${usuario.toLowerCase()}`,20);
  const [u]=await sql`SELECT * FROM usuarios WHERE lower(email)=${usuario.toLowerCase()} OR (email IS NULL AND usuario=${usuario}) LIMIT 1`;
